...
func (m *CombinerMultiplexer) GetActiveDirectivesForContext(ac agentic_context.AgentContext) []directive.Directive {
	return m.collectActive(ac)
}
...
func (m *CombinerMultiplexer) collectActive(ac agentic_context.AgentContext) []directive.Directive {
	seen := make(map[string]struct{})
	var result []directive.Directive

	for _, mx := range m.multiplexers {
		for _, d := range mx.GetActiveDirectivesForContext(ac) {
			name := d.GetName()
